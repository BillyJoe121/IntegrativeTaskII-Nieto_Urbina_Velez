...
def evaluate_rnn(X_train, X_test, y_train, y_test):
    ...
    # Training history
    plot_training_history(
        history, "Vanilla RNN", save_path=str(FIGURES_DIR / "rnn_training_history.png")
    )
...
def evaluate_lstm(X_train, X_test, y_train, y_test):
    ...
    # Confusion matrix
    plot_confusion_matrix(
        y_test, y_pred, "LSTM", save_path=str(FIGURES_DIR / "lstm_confusion_matrix.png")
    )

    # Training history (if available)
    if history is not None:
        plot_training_history(
            history, "LSTM", save_path=str(FIGURES_DIR / "lstm_training_history.png")
        )
    else:
        print(" Skipping training history plot (history not available)")
